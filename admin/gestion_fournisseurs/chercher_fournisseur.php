...
{     
      ...
      $sql = "SELECT * FROM fournisseurs WHERE login LIKE '%$requete_user%'";
      $result = mysqli_query($conn, $sql);
        ...
        <table class='table table-hover table-bordered'>
          ...
          echo'<tr>';
          echo'<td >' . $row['idf'] . '</td><p>';
          echo'<td>' . $row['login'] . '</td><p>';
          echo'<td>' . $row['societe'] . '</td><p>';
          echo '<td>';
echo '<a class="btn btn-secondary"  href="modifier_fournisseur.php?id= '  . $row['idf'] . ' " > 
      Modifier Fournisseur   </a> ';
echo '</td>';

echo'<td>';
echo'<a class="btn btn-danger" href="supprimer_fournisseur.php?id= ' . $row['idf'] . ' " >  Supprimer Fournisseur   </a>  ';
echo '</td>';
...
<div class="container">  
<center>
    <h2> CHERCHER UN FOURNISSEUR  DANS L'APPLICATION </h2> 
</center>               
...
<form action="chercher_fournisseur.php" method="GET" >
