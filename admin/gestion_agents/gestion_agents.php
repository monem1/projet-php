...
<div class="container"> 
<center>
    <h2> TOUS LES AGENTS  DE L 'APPLICATION </h2>  
</center>

<a href="../home.php"class="btn btn-dark float-right"> Home </a>
						<br><br>
<a href="chercher_agent.php" class="btn btn-warning float-right">search </a> 
 						<br>

<a href="ajouter_agent.php" class="btn btn-primary">Ajouter Agent </a>
...
// Préparation de la requête avec le LIMIT
	$sql = 'SELECT * FROM agents ORDER BY ida DESC LIMIT '.$_GET['debut'].','.$nb_affichage_par_page;
...
// on va scanner tous les tuples un par un
	...
	echo'<tr>';
echo'<td >' . $data['ida'] . '</td><p>';
echo'<td>' . $data['login'] . '</td><p>';
echo'<td>' . $data['fonction'] . '</td><p>';
echo'<td>' . $data['type'] . '</td><p>';
// un autre td pour le bouton de modifier
echo '<td>';
echo '<a class="btn btn-secondary"  href="modifier_agent.php?id= '  . $data['ida'] . ' " > Modifier Agent   </a> ';
echo '</td>';

// un autre td pour le bouton de suppression
echo'<td>';
echo'<a class="btn btn-danger" href="supprimer_agent.php?id= ' . $data['ida'] . ' " >  Supprimer Agent   </a>  ';
echo '</td>';
echo '</tr>';
	}
